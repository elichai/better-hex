//! Uppercase hex serde support (fast SIMD path, no prefix).
//!
...
/// Serialize bytes as uppercase hex (fast SIMD path, no prefix).
...
/// Deserialize bytes from an uppercase (or mixed-case) hex string (fast path, no prefix).
///
